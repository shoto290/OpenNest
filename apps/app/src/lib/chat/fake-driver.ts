...
function defaultReply(prompt: string): string {
	return `Simulated answer to "${prompt}".\n\nThis text is streamed piece by piece by the fake driver to reproduce a full Claude Code turn.\n\nEach paragraph joins the thread as soon as it is finished.`
}
...
export function createFakeChatDriver(
			...
			steps.push(() => {
				emit({
					...
					error: {
						kind: "crashed",
						code: 1,
						detail: "Simulated fake driver failure",
					},
				})
				finishTurn("failed")
			})
	...
	return {
		...
		respondToPermission: (id: string, decision: PermissionDecision) => {
			...
			// A denial does not end the turn: the tool comes back with an error and
			// Claude keeps going, so the turn still ends `completed`.
			waiting = false
			pump()
			return Promise.resolve()
		},
