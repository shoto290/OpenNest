...
Read the choices with `companion_suggestions`, then ask exactly one `AskUserQuestion`: what they want a companion for this week, one real task. Its options are those suggestions, in the order answered, the name as the label and the blurb as the description. Nothing else is asked at that point.
