...
export const SpaceFlickMomentum = meta.story({
	...
	parameters: {
		docs: {
			description: {
				story:
					"A trackpad flick, which a browser reports as two scrolls rather than one: the fingers leave the glass and the scroll ends there, then the momentum they left carries the row on and ends a second time. Waiting for that second end is waiting seconds for inertia to die on a panel that has been flush the whole time, so what makes a landing is the panel being in place, not the row being still — a space flush in the viewport is reported the moment it gets there, momentum or not, and reported once however many frames it is carried through. Between panels there is nothing to report: stopped short of half a panel it reports nothing and stays where it stopped, and stopped past half — the moment the row used to call a crossing — it still reports nothing and still stays where it stopped, same drawn window, same offset. Check the flush panel is the landing and the only one: the space is reported there, once, the reader is left on the panel the row came to rest on, one space along and no further, and the row is not slid out from under a live gesture — the drawn window only follows on the rest. Check a rank chord still slides the row from there. Pick `SpaceScrolling` for the snapping the flick rides on, `SpaceLanding` for the walk from flush panel to flush panel, `SpaceSwipeTakenBack` for a flick that never lands anywhere new.",
			},
		},
	},
	play: async ({ args, canvasElement, userEvent }) => {
